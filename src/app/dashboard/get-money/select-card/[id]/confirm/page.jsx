...
const ConfirmPage = () => {
    ...
    const { selectedCardData, selectedCardId, getCard } = useCards();
    const { accountDetails, fetchAccountDetails } = useAccount();
    const { depositAmount, loading, error, success ,setLoading,setSuccess} = useTransaction();
    ...
    useEffect(() => {
        if (accountDetails?.id && selectedCardId) {
            getCard(accountDetails?.id, selectedCardId);
        }
    }, [accountDetails?.id, selectedCardId]);

    const handleDeposit = async () => {
        if (!accountDetails?.id || !selectedCardData) return;
